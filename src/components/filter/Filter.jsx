...
"  style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
                ...
                <div className="flex items-center justify-between mt-4">
                    ...
                    <button onClick={()=>{setFilterPrice("");setFilterType("");}} className="px-4 py-2 bg-gray-50hover:bg-gray-200 text-gray-800 text-sm font-medium rounded-md" style={{ color: mode === 'dark' ? 'white' : '' }}>
                        Reset Filter
                    </button>
                </div>
                <div>
        ...
        {Array.from(new Set(product.map(item => item.category))).map((category, index) => (
            <option key={index} value={category}>
                 {category}
                        ...
                        <option value="">Price</option>{product.map((item, index) => {
                                return (
